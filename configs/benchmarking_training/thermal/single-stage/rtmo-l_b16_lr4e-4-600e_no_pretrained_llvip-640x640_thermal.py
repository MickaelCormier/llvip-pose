...
work_dir = "runs/benchmarks/thermal/rtmo-l_b16_lr4e-4_no_pretrained_thermal"
...
default_hooks = dict(checkpoint=dict(save_best="coco/AP", rule="greater", interval=20, max_keep_ckpts=5))
...
# data
input_size = (640, 640)
metafile = "configs/_base_/datasets/llvip.py"
codec = dict(type="YOLOXPoseAnnotationProcessor", input_size=input_size)

train_pipeline_stage1 = [
    dict(type="LoadImage", backend_args=None),
    dict(
        type="Mosaic", img_scale=(640, 640), pad_val=114.0, pre_transform=[dict(type="LoadImage", backend_args=None)]
    ),
    dict(
...
val_evaluator = dict(
    type="CocoMetric",
    ann_file=data_root + "annotations/test_llvip.json",
    score_mode="bbox",
    nms_mode="none",
    iou_type="keypoints"
)
...
custom_hooks = [
    ...
    dict(
        type="EMAHook", ema_type="ExpMomentumEMA", momentum=0.0002, update_buffers=True, strict_load=False, priority=49
    ),
]
...
model = dict(
    ...
    head=dict(
        ...
        assigner=dict(
            type="SimOTAAssigner", dynamic_k_indicator="oks", oks_calculator=dict(type="PoseOKS", metainfo=metafile)
        ),
