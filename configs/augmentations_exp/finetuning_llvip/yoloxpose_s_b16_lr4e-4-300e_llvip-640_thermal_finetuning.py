...
work_dir = "runs_conference/yolopose"

# runtime
train_cfg = dict(
    _delete_=True, type="EpochBasedTrainLoop", max_epochs=300, val_interval=10, dynamic_intervals=[(280, 1)]
)

auto_scale_lr = dict(base_batch_size=256)

default_hooks = dict(checkpoint=dict(save_best="coco/AP", rule="greater", interval=10, max_keep_ckpts=5))
...
model = dict(
    ...
    head=dict(
        ...
        loss_oks=dict(
            type="OKSLoss",
            reduction="none",
            metainfo="configs/_base_/datasets/llvip.py",
...
val_evaluator = dict(
    type="CocoMetric",
    ann_file=data_root + "annotations/finetuning/test_llvip_finetuning_13_kpts.json",
    score_mode="bbox",
    nms_mode="none",
)
